...
async function isServerOnline() {
    try {
        const res = await fetch(`http://localhost:1236/flair/api/v1/setup`, { method: 'HEAD' });
        return res.ok;
    } catch (e) {
        return false;
    }
...
async function GET(endpoint: string, params: object) {
    ...
    const url = `http://localhost:1236/flair/api${endpoint}?${query.join('&')}`;
...
async function PUT(endpoint: string, params: object, jwt: string) {
    const url = `http://localhost:1236/flair/api${endpoint}`;
